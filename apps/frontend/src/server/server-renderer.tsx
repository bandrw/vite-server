...
import {ServerAppContext} from './server-app';
...
const createServerRendererApp = async () => {
    ...
    const clientAppEntryPoint = await (async () => {
        if (isProduction) {
            const fileName = (await import('../../build/client/manifest.json')).default['src/client/client-app.tsx']
                .file;
    ...
    app.get<unknown, unknown, unknown, unknown, ServerAppContext>('*', async (req, res) => {
        ...
        const {pipe, abort} = ReactDOM.renderToPipeableStream(
            <html lang="en">
                ...
                {isProduction ? (
                    <script type="module" src={clientAppEntryPoint} />
                ) : (
                    <script type="module" src={clientAppEntryPoint} />
                )}
            </html>,
            {
                ...
                onShellError(err) {
                    res.statusCode = 500;
                    res.send(ReactDOM.renderToString(<div>500 error page</div>));
                    console.error(err);
                },
            },
        );
